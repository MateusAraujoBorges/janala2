...
package tests.casestudies.sir.jtopasV0;
...
import janala.Main;
import tests.casestudies.sir.jtopasV0.util.Token;
import tests.casestudies.sir.jtopasV0.util.Tokenizer;
import tests.casestudies.sir.jtopasV0.util.TokenizerException;
import tests.casestudies.sir.jtopasV0.util.TokenizerProperty;
 ...
 * @throws Throwable 
  ...
  public static void main(String[] args) throws Throwable {
	  TestPluginTokenizer tk = new TestPluginTokenizer();
	  tk.testContentsParsing();
  }
...
//  
  ...
  public void testContentsParsing() throws Throwable {
	...
	final char[] input = new char[N];
    for (int i = 0; i < N; i++) {
        char x = Main.readChar('a');
  ...
  private static final TokenizerProperty  AUML_TAG 
    = new TokenizerProperty(Token.SPECIAL_SEQUENCE, new String[] { "&auml;" }, "\\F4", Tokenizer.F_NO_CASE );

  private static final TokenizerProperty  OUML_TAG 
    = new TokenizerProperty(Token.SPECIAL_SEQUENCE, new String[] { "&ouml;" }, "\\F6", Tokenizer.F_NO_CASE );

  private static final TokenizerProperty  UUML_TAG 
    = new TokenizerProperty(Token.SPECIAL_SEQUENCE, new String[] { "&uuml;" }, "\\FC", Tokenizer.F_NO_CASE );

  private static final TokenizerProperty  SZLIG_TAG 
    = new TokenizerProperty(Token.SPECIAL_SEQUENCE, new String[] { "&szlig;" }, "\\DF", Tokenizer.F_NO_CASE );
}