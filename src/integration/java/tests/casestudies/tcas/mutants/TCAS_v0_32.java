package tests.casestudies.tcas.mutants;


public class TCAS_v0_32 {
...
//	 @Symbolic("true")
	...
	private boolean Non_Crossing_Biased_Climb(){
	    ...
	    if (upward_preferred){
	    	result = !(Own_Below_Threat()) || ((false));
	    }else{	
	    	result = Own_Above_Threat() && (Cur_Vertical_Sep >= MINSEP) && (Up_Separation >= ALIM());
	    }
			
//	    assert ((this.Other_Tracked_Alt != tcas.TCAS_v0.DO_NOT_CLIMB) || !(result == false));
//	    assert ((this.Other_Tracked_Alt != daikon.Quant.getElement_int(this.Positive_RA_Alt_Thresh, tcas.TCAS_v0.DO_NOT_DESCEND)) || !(result == false));
//	    assert ((this.Other_Tracked_Alt != daikon.Quant.getElement_int(this.Positive_RA_Alt_Thresh, this.Other_Capability)) || !(result == false));
//	    assert (this.Other_Capability < daikon.Quant.getElement_int(this.Positive_RA_Alt_Thresh, tcas.TCAS_v0.DO_NOT_DESCEND));
//	    assert (this.Other_Capability < daikon.Quant.getElement_int(this.Positive_RA_Alt_Thresh, this.Other_Capability));
//	    assert (daikon.Quant.size(this.Positive_RA_Alt_Thresh) < daikon.Quant.getElement_int(this.Positive_RA_Alt_Thresh, this.Other_Capability));
    ...
    public static void main (String[] args) {
    	int res =0;
        TCAS_v0_32 tcas = new TCAS_v0_32();
        if (args.length == 12){
