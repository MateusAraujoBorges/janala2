...
public class Main {
  ...
  public static void MakeSymbolicLong(long x, long lo, long hi) {
  }
  ...
  public static void setRealInput(boolean isReal) {
    ...
    try {
      if (isRealInput) {
        writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("isRealInput"), "utf-8"));
        writer.write("" + isReal);
        isRealInput = isReal;
      }
  ...
  public static void event(String eventName) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (pathsState == null) {
        System.out.println(
            "Pruning path as event prefix '" + eventPrefix + "' is not in regular expression '" + pathRegex + "'");
