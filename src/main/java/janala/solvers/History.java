...
/**
 * A collection of import check points (element) and the corresponding
 * constraints
 */
public class History {
  ...
  public static void createBackTrackHistory(int skipIndex, String fileName) throws Exception {
    History.createBacktrackHistory(skipIndex, new FileInputStream(fileName), new FileOutputStream(fileName + ".bak"));
  }
  ...
  public void beginScope(int iid) {
    ...
    if (index < history.size()) {
      ...
      if (isEnd(tmp)) {
        ...
        logger.log(Level.WARNING, "!!!!!!!!!!!!!!!!! Prediction failed !!!!!!!!!!!!!!!!! index " + index
  ...
  public void checkAndSetBranch(boolean result, Constraint constraint, int iid) {
    ...
    if (index < history.size()) {
      ...
      if (isEnd(tmp) || ignore) {
        ...
        history.add(index, current);
      } else if (!ignore && (!(tmp instanceof BranchElement) || ((BranchElement) tmp).getBranch() != result)) {
        predictionFailed = true;
        ...
        logger.log(Level.WARNING, "At old iid " + tmp.getIid() + " at iid " + iid + " constraint " + constraint);
        current = new BranchElement(result, false, -1, iid);
        clearAndSet(current);
      } else {
  ...
  public void addInput(int symbol, Value value, int lo, int hi) {
    inputs.add(new InputElement(symbol, value, lo, hi));
  }

  public void addInput(int symbol, Value value, long lo, long hi) {
    inputs.add(new InputElement(symbol, value, lo, hi));
  }
