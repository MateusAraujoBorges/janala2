...
public class BoundedDomainSolverWrapper implements Solver {
  ...
  private List<Constraint> generateDomainConstraints(List<InputElement> inputs) {
    ...
    for (InputElement input : inputs) {
      ...
      lowerBound = lowerBound.setop(COMPARISON_OPS.GE);
      long lo = bound.lowerEndpoint().equals(Integer.MIN_VALUE) ? Integer.MAX_VALUE : bound.lowerEndpoint() * -1;
      lowerBound = lowerBound.add(lo);

      SymbolicInt upperBound = new SymbolicInt(input.symbol);
      upperBound = upperBound.setop(COMPARISON_OPS.LE);
      upperBound = upperBound.add(bound.upperEndpoint() * -1);
