...
public interface SymbolicTree extends Serializable {
  ...
  /**
   * Update the tree with new constraints. New nodes will be created as needed.
   * This method also can be used to search for nodes. Intermediary
  ...
  public BigRational count(List<SymbolicCountNode> path, List<InputElement> inputs, Map<Integer, Value> syntheticVars,
      Counter counter);

  /**
   * Update count estimates and remove nodes without remaining unexplored paths
   * from the tree. This happens in two steps:
   * 
   * - The last element of {@code path} is pruned from the tree. This element
   * must be counted and have only {@code PrunedNode}s as childs. - Every other
   * path element, in reverse order, will have their number of solutions/
   * probability updated, and will be pruned too if this number is equal to
   * zero.
   * 
   * @param path
   */
