...
public class CVC4Solver implements Solver {
  ...
  public static class Printer {
    ...
    public Printer(Set<String> freeVars, Map<String, Long> sol, CONSTRAINT_TYPE type, PrintStream out) {
  ...
  private void print(Constraint con, PrintStream out, Set<String> freeVars, CONSTRAINT_TYPE type,
  ...
  public boolean printFormula(PrintStream out, Map<String, Long> soln, Set<String> freeVars, String extra,
    ...
    // This behavior is annoying when we are not following an dfs-like concolic
    // execution, so I added an option (setNegateLast) to control it.

    // System.out.println("Constraint "+pathConstraintIndex+": !" +
    // constraints.get(pathConstraintIndex));
    Constraint notCon;
    if (negateLast) {
      notCon = constraints.get(pathConstraintIndex).not().substitute(soln);
    } else {
      notCon = constraints.get(pathConstraintIndex).substitute(soln);
    }
  ...
  private RESULT_TYPE writeFormula(String extra, CONSTRAINT_TYPE type, TreeMap<String, Long> soln) {
    try {
      PrintStream out = new PrintStream(new BufferedOutputStream(new FileOutputStream(config.formulaFile + ".tmp")));
      ...
      concatFile(freeVars, config.formulaFile + ".tmp", config.formulaFile, true);
    ...
    try {
      ...
      if (!line.startsWith("sat")) {
        if (!line.contains("unsat")) {
          ...
          logger.log(Level.SEVERE, "Call to CVC4 failed (concolic.cvc4 = " + config.cvc4Command + ")");
          Runtime.getRuntime().halt(1);
        }
  ...
  private String solve(String extra, CONSTRAINT_TYPE type, TreeMap<String, Long> soln) {
    try {
      ...
      ProcessBuilder builder = new ProcessBuilder(
          new String[] { config.cvc4Command, "--lang", "cvc4", config.formulaFile });
  ...
  // VisbleForTest
  public void concatStreams(List<PrintStream> ps, Set<String> freeVars, String from, boolean cvc4)
  ...
  public void concatFile(LinkedHashSet<String> freeVars, String from, String to, boolean cvc4)
      throws java.io.IOException {
