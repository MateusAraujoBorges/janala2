...
public class ChocoSolver implements janala.solvers.Solver {
  ...
  private SymbolicInt initSolver(SymbolicInt c) {
    if (first) {
      ...
      vars = new IntVar[len];
      for (int i = 0; i < len; i++) {
        vars[i] = VariableFactory.integer("x" + i, VariableFactory.MIN_INT_BOUND, VariableFactory.MAX_INT_BOUND,
            solver);
      }
      return c.not();
    }
  ...
  public void visitSymbolicOr(SymbolicOrConstraint or) {
    org.chocosolver.solver.constraints.Constraint[] con = new org.chocosolver.solver.constraints.Constraint[or.constraints
        .size()];
    int idx = 0;
