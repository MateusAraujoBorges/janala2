...
public final class SymbolicInt extends Constraint {
  ...
  public SymbolicInt(int i) {
    linear = new LinkedHashMap<Integer, Long>();
    linear.put(i, 1L);
    constant = 0;
  ...
  private SymbolicInt(SymbolicInt e) {
    this.linear = new LinkedHashMap<Integer, Long>(e.linear);
    constant = e.constant;
    op = e.op;
  }

  public SymbolicInt negate() {
    Map<Integer, Long> tmpMap = new LinkedHashMap<Integer, Long>(getLinear());
  ...
  private SymbolicInt add(long l, boolean add) {
    ...
    return new SymbolicInt(linear, tmpConstant, op);
  }
  ...
  private SymbolicInt add(SymbolicInt l, boolean add) {
    Map<Integer, Long> tmpLinear = new LinkedHashMap<Integer, Long>(linear);
  ...
  public SymbolicInt multiply(long l) {
    ...
    if (l == 1)
      return this;
    Map<Integer, Long> tmpMap = new LinkedHashMap<Integer, Long>();
  ...
  public Constraint substitute(Map<String, Long> assignments) {
    ...
    for (Map.Entry<Integer, Long> it : linear.entrySet()) {
      ...
      if (assignments.containsKey("x" + key)) {
        ...
        isSymbolic = true;
        if (retLinear == null) {
          retLinear = new LinkedHashMap<Integer, Long>();
        }
        retLinear.put(key, l);
      }
    }
