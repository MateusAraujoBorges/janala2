...
public final class StaticInvocation {
  ...
  public Value invokeMethod(int iid, String owner, String name, Value[] args, History history) {
    if (owner.equals("java/lang/Integer") && name.equals("valueOf")) {
      ...
      return PlaceHolder.instance;
    } else if (owner.equals("janala/Main") && name.equals("MakeSymbolic") && args.length == 3) {
      int symbol = args[0].MAKE_SYMBOLIC(history);
      int lo = ((IntValue) args[1]).concrete;
      ...
      int symbol = args[0].MAKE_SYMBOLIC(history);
      long lo = ((LongValue) args[1]).getConcreteLong();
      long hi = ((LongValue) args[2]).getConcreteLong();
      history.addInput(symbol, args[0], lo, hi);
      return PlaceHolder.instance;
    }

    else if (owner.equals("janala/Main") && name.equals("BeginScope") && (args == null || args.length == 0)) {
      history.addInput(config.scopeBeginSymbol, null);
      history.beginScope(iid);
      return PlaceHolder.instance;
    } else if (owner.equals("janala/Main") && name.equals("EndScope") && (args == null || args.length == 0)) {
