...
public class RegexpEncoder {
  ...
  public static Constraint getLengthFormulaString(String regexp, String prefix, int sym, boolean accept) {
    ...
    if (example != null)
      return intCompare(prefix, sym, example.length(), COMPARISON_OPS.GE);
    else
      return intCompare(prefix, sym, 0, COMPARISON_OPS.GE);
  }
  ...
  private static Constraint intCompare(String prefix, int sym, int constant, COMPARISON_OPS op) {
  ...
  public static Constraint createFormula(State root, String prefix, int i, int n) {
    ...
    for (Transition transition : transitions) {
      ...
      Constraint tmp1 = andFormula(intCompare(prefix, i, transition.getMin(), COMPARISON_OPS.GE),
          intCompare(prefix, i, transition.getMax(), COMPARISON_OPS.LE));
