...
public class SymbolicStringPredicate extends Constraint {
  ...
  public STRING_COMPARISON_OPS getOp() {
    return op;
  }
  ...
  private SymOrInt exprAt(Object sExpr, int i, Set<String> freeVars, Map<String, Long> assignments) {
  ...
  private Constraint getStringEqualityFormula(Object left, Object right, long length, Set<String> freeVars,
    ...
    for (int i = 0; i < length; i++) {
      ...
      Constraint c;
      c = new SymbolicIntCompareConstraint(e1, e2, COMPARISON_OPS.EQ);
  ...
  private IntValue getLength(Object s) {
    return (s instanceof String) ? new IntValue(((String) s).length())
        : ((SymbolicStringExpression) s).getField("length");
  }

  public Constraint getFormula(Set<String> freeVars, CONSTRAINT_TYPE mode, Map<String, Long> assignments) {
    ...
    if (mode == CONSTRAINT_TYPE.INT) {
      ...
      case NE:
        ...
        // Essentially it checks that if left and right size are nonempty
        // strings.

        SymbolicInt int1 = s1.symbolic != null ? s1.symbolic.setop(COMPARISON_OPS.GT) : null;
        SymbolicInt int2 = s2.symbolic != null ? s2.symbolic.setop(COMPARISON_OPS.GT) : null;
      ...
      case NOTIN:
        ...
        // @todo regex_escape
        return RegexpEncoder.getRegexpFormulaString("~(" + (String) this.right + ")", "x" + this.left + "__",
            (int) length1);
      }
    }
