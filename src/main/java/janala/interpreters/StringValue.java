...
public final class StringValue extends ObjectValue {
  ...
  public boolean equals(Object o) {
    if (o == null) {
      ...
      return (this.string == other.string || this.string.equals(other.string))
          && (this.symbolicExp == other.symbolicExp || this.symbolicExp.equals(other.symbolicExp));
    } else {
      return false;
    }
  }
    ...
    if (arg instanceof StringValue) {
      ...
      if (symbolicExp != null && other.symbolicExp != null) {
        ...
        return new IntValue(result ? 1 : 0,
            new SymbolicStringPredicate(SymbolicStringPredicate.STRING_COMPARISON_OPS.EQ, symbolicExp, other.string));
      } else if (other.symbolicExp != null) {
        return new IntValue(result ? 1 : 0,
            new SymbolicStringPredicate(SymbolicStringPredicate.STRING_COMPARISON_OPS.EQ, string, other.symbolicExp));
      } else {
        return new IntValue(result ? 1 : 0);
      }
    } else {
  ...
  private Value invokeStartsWith(Value[] args) {
    if (args.length == 1) {
      if (args[0] instanceof StringValue) {
        ...
        if (symbolicExp != null) {
          return new IntValue(result ? 1 : 0, new SymbolicStringPredicate(
              SymbolicStringPredicate.STRING_COMPARISON_OPS.IN, symbolicExp, escapeRE(other.string) + ".*"));
        } else {
          return new IntValue(result ? 1 : 0);
        }
      }
      ...
      if (args[0] instanceof StringValue) {
        ...
        if (symbolicExp != null) {
          return new IntValue(result ? 1 : 0,
              new SymbolicStringPredicate(SymbolicStringPredicate.STRING_COMPARISON_OPS.IN, symbolicExp,
  ...
  private Value invokeEndsWith(Value arg) {
    ...
    if (symbolicExp != null) {
      return new IntValue(result ? 1 : 0, new SymbolicStringPredicate(SymbolicStringPredicate.STRING_COMPARISON_OPS.IN,
          symbolicExp, ".*" + escapeRE(other.string)));
    } else {
      return new IntValue(result ? 1 : 0);
    }
  }
  ...
  private Value invokeMatches(Value arg) {
    ...
    if (symbolicExp != null) {
      return new IntValue(result ? 1 : 0,
          new SymbolicStringPredicate(SymbolicStringPredicate.STRING_COMPARISON_OPS.IN, symbolicExp, other.string));
    } else {
      return new IntValue(result ? 1 : 0);
    }
  }
  ...
  public int MAKE_SYMBOLIC(History history) {
    ...
    results = length.ISUB(new IntValue(Config.instance.maxStringLength)).symbolic.setop(COMPARISON_OPS.LE);
